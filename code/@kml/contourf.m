...
%   $Revision: 2.3 $  $Date: 2012/09/05 08:00:00 $
    ...
    p.addParamValue('caxis',@(a)numel(a)==2);
    ...
    if isnumeric(arg.colorMap)
        arg.numberOfLevels = length(arg.colorMap);
    end
   
    if numel(arg.caxis) == 2
        levels = linspace(arg.caxis(1), arg.caxis(2), arg.numberOfLevels+1);
        minAlt = levels(1);
    end
    ...
    % lea mueller, 20150129, add caxis for colormap
    if numel(arg.caxis) == 2
        levels = linspace(arg.caxis(1), arg.caxis(2), arg.numberOfLevels+1);
        minAlt = levels(1)*0.9;
    else
    ...
    %% lea mueller, 20150128, add functionality to input colormap as array instead of char
    if ischar(arg.colorMap)
        ncolors = 100;
